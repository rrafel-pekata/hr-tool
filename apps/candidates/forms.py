...
class CandidateCreateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'first_name': forms.TextInput(attrs={'class': INPUT_CLASS, 'placeholder': 'Nombre'}),
            'last_name': forms.TextInput(attrs={'class': INPUT_CLASS, 'placeholder': 'Apellidos'}),
            'email': forms.EmailInput(attrs={'class': INPUT_CLASS, 'placeholder': 'email@ejemplo.com'}),
            'phone': forms.TextInput(attrs={'class': INPUT_CLASS, 'placeholder': '+34 600 000 000'}),
...
class CandidateEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'recruiter_notes': forms.Textarea(attrs={'class': INPUT_CLASS, 'rows': 4, 'placeholder': 'Notas rápidas sobre el candidato...'}),
